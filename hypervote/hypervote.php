...
function generator($ig, $climate)
{
    ...
    $climate->out('');
    sleep(2);
    $climate->out('Please provide a valid license key from your Dashboard on SocialMediaTools.eu (https://socialmediatools.eu/).');
    sleep(1);
    ...
    do {
        foreach ($data_targ as $key => $d) {
            try {
                ...
                if (0 === $filter_c || 2 === $filter_c && $step1) {
                    try {
                ...
                do {
					...
					$mlv2_speed =  $data->masslookingv2_speed ? $data->masslookingv2_speed : 5000;
							 if (!$masslookingv2_verified) {
								 ...
								 $index_old2 = 0;
								  $index_new2 += 20;
                    if (!isset($followers_ids[$index_new2])) {
                        do {
                            $index_new -= 20;
                        } while (!isset($followers_ids[$index_new2]));
                    }
                    if ($index_new2 < $index_old2) {
                        break;
                    }
                    $ids2 = [];
                    ...
                    for ($i = $index_old2; $i <= $index_new2; $i++) {
                        if (isset($followers_ids[$i])) {
                           
                                $ids2[] = $followers_ids[$i];
                 ...
                 if ($masslookingv2 && $mass_view_v2)  {
					  ...
					  try {
                                        ...
                                        $stories_reels2 = $ig->story->getReelsMediaFeed($ids2);
										
										 $now_ms = time();
                                         if ($now_ms - $begin_ms >= 13) {
                                                                // All fine
                                                            } else {
                                                                $sleep =  (13 - ($now_ms - $begin_ms));
                                                                sleep($sleep);
                                                            } $begin_ms = time();
                                ...
                                if (isset($stories_reels2) && $stories_reels2->isOk() && count($stories_reels2->getReels()->getData()) > 0) {
                                    // Save user story reels's to array
                                    $reels2 = [];
                                    $reels2 = $stories_reels2->getReels()->getData();
    
                                    foreach ($reels2 as $r2) {
                                        $items2 = [];
                                        $stories_loop = [];
                                        $items2 = $r2->getItems();
                                        ...
                                        $latest_reel_media2 = $r2->getLatestReelMedia();
                                        $seen2 = $r2->getSeen();
    
                                        if (($latest_reel_media2 !== "0")) {
                                            foreach ($items2 as $it) {
                                                if (($it->getTakenAt() == $latest_reel_media2) && ($it->getId()) !== "0") {
                                                    $stories_loop[] = $it;
                                                }
                                            }
                                        }
                                        ...
                                        if (count($stories_loop) > 0) {
                                            ...
                                            if ($st_count >= 200) {
                                                ...
                                                $now_ms = time();
                                                 if ($now_ms - $begin_ms >= 5) {
                                                    // all fine
                                                } else {
                                                    $counter3 = 5 - ($now_ms - $begin_ms) + rand(1, 3);
                    ...
                    try {
                        try {
                            ...
                            $stories_reels = $ig->story->getReelsMediaFeedWeb($ids);
							 $now_ms = time();
                                         if ($now_ms - $begin_getreel >= 5) {
                                                                // All fine
                                                            } else {
                                                                $sleep =  (5 - ($now_ms - $begin_getreel));
                                                                sleep($sleep);
                                                            } $begin_getreel = time();
                            ...
                            */
                            ...
                            if ($votableStoryUsers) {
                                foreach ($votableStoryUsers as $userId) {
                                    try {
										$stories_reels = null;
										        $getUserFriendship3 = $ig->people->getFriendship($userId);
															
												$getUserHighlight = $ig->timeline->getUserFeed($userId);
                                                $getUserHighlight = $ig->highlight->getUserFeed($userId);
                                                                                    ...
                                                                                    if ($user_feed->getNumResults() > 0) {
                                                                                        ...
                                                                                        foreach ($user_feed->getItems() as $key => $uf_item) {
                                                                                            if ($uf_item->getId() && !$uf_item->getHasLiked()) {
                                                                                                ...
                                                                                                $like_response = $ig->media->like($post_id);
								...
								if ($is_follow && $is_follow_active) {
                                                                    if ( $follow_sleep) {
                                                                        ...
                                                                        try {
                                                                            ...
                                                                            $follow_response = $ig->people->follow($item->user->pk);
